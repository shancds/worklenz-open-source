...
interface ProjectInviteTokenPayload {
  projectId: string;
  userId: string;
  type: "project_invite";
  expiresAt: number;
...
class TokenService {
  ...
  // Generate project invitation token
  generateProjectInviteToken(payload: ProjectInviteTokenPayload): string {
    return jwt.sign(payload, this.INVITE_SECRET, {
      expiresIn: "1d", // Project invitations expire in 1 days
      issuer: "worklenz-project-invite",
      audience: "project_invite"
    });
  }
